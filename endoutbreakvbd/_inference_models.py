...
def _build_underreporting_model(
    *,
    ...
    reporting_prob_vec = np.clip(
        _reporting_prob_vec(observed_incidence_vec, reporting_prob, delay_cdf),
        1e-6,
        1.0,
    )
    ...
    with model:
        ...
        def _dist(rep_no_data: Any, size: Any) -> Any:
            ...
            return pm.Poisson.dist(
                mu=np.maximum(
                    observed_incidence_after_index_vec
                    * (1 - reporting_prob_after_index_vec)
                    / reporting_prob_after_index_vec,
                    1e-3,
                ),
                shape=t_data_stop - 1,
            )
